...
import com.okg.wikipedia.message.communication.{MigrationCompleted, StartSimulation, TerminateSimulation}
import com.okg.wikipedia.message.registration.{CompilerRegistrationAtInstance, StatisticsRegistrationAtInstance}
import com.okg.wikipedia.message.statistics.{InstanceStatistics, Load}
...
class InstanceActor(index: Int) extends Actor with FSM[InstanceState, InstanceStateData] {
  ...
  startWith(RUN, new InstanceStateData(0, 0, mutable.Map.empty[String, Int], null))

  var receivedPeriodBarriersNum = 0
  when(RUN) {
    case Event(tuple: Tuple[String], data: InstanceStateData) => {
      // process tuple
