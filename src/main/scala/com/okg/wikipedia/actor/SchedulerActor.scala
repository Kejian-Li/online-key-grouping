...
import com.okg.wikipedia.message.communication._
import com.okg.wikipedia.message.registration.StatisticsRegistrationAtScheduler
import com.okg.wikipedia.message.statistics.SchedulerStatistics
import com.okg.wikipedia.state._
import com.okg.wikipedia.tuple.{Tuple, TupleQueue}
import com.okg.wikipedia.util.{SpaceSaving, TwoUniversalHash}
...
class SchedulerActor(index: Int, // index of this Scheduler instance
  ...
  def assignTuple(tuple: Tuple[String], routingTable: RoutingTable) = {
    var targetIndex = -1
    val key = tuple.key
  ...
  def makeSketch(rawHeavyHitters: util.HashMap[String, Integer], buckets: Array[Int]) = {
    val it = rawHeavyHitters.entrySet().iterator()
    val heavyHitters = new mutable.HashMap[String, Int]()
